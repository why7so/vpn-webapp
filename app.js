(function () {
  ...
  function renderRing(daysLeft) {
    ...
    // повёрнутый к центру.
    // Шаг между штрихами по окружности ≈ 10.9; при ширине 5.4 просвет
    // равен штриху — кольцо плотное, но штрихи ещё различимы.
    const tickW = 5.4;
    const tickL = 9;
  ...
  // viewBox выровнен по ЦЕНТРУ МАСС фигуры, а не по её габаритам и тем более
  // не оставлен 0 0 24 24. Габариты дают геометрическую середину, но глаз
  // ловит середину площади: у карандаша тело толще у нижнего конца, а остриё
  // тонкое, и центр масс (11.89, 12.11) лежит на 0.61 ниже и левее центра
  // габаритов (12.5, 11.5). Сместив на эту разницу, получаем значок, который
  // выглядит по центру, а не только измеряется по центру.
  //
  // Сторона 20 у обоих — чтобы значки остались одного масштаба.
  const ICON_PENCIL =
    '<svg viewBox="1.89 2.11 20 20" fill="currentColor">' +
  ...
  // То же для корзины: корпус занимает заметно больше площади, чем крышка,
  // поэтому центр масс (12, 12.77) на 0.27 ниже центра габаритов.
  const ICON_TRASH =
